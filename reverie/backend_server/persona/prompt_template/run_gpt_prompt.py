...
def run_gpt_prompt_action_arena(
    ...
    if output not in x:
        logger.warning(f"LLM response `{output}` not in {x}, so random select one of them.")
        output = random.choice(x)
...
def run_gpt_prompt_create_conversation(
    ...
    def create_prompt_input(init_persona, target_persona, curr_loc, test_input=None):
        ...
        if init_persona.a_mem.seq_chat:
            for i in init_persona.a_mem.seq_chat:
                if i.object == target_persona.name:
                    v1 = int(
                        (init_persona.curr_time - i.created).total_seconds()
                        / 60
                    )
                    prev_convo_insert += (
        ...
        prompt_input = []
        prompt_input += [init_persona.iss]
        prompt_input += [target_persona.iss]
        ...
        prompt_input += [init_persona.curr_time.strftime("%B %d, %Y, %H:%M:%S")]
...
def run_gpt_prompt_agent_chat(
    ...
    def create_prompt_input(
        ...
        if persona.a_mem.seq_chat:
            for i in persona.a_mem.seq_chat:
                if i.object == target_persona.name:
                    v1 = int(
                        (persona.curr_time - i.created).total_seconds() / 60
                    )
                    prev_convo_insert += f"{str(v1)} minutes ago, {persona.name} and {target_persona.name} were already {i.description} This context takes place after that conversation."
                    break
        if prev_convo_insert == "\n":
            prev_convo_insert = ""
        if persona.a_mem.seq_chat:
            if (
                int(
                    (
                        persona.curr_time - persona.a_mem.seq_chat[-1].created
                    ).total_seconds()
                    / 60
                )
                > 480
                ...
                prev_convo_insert = ""
        logger.debug(f"prev_convo_insert: {prev_convo_insert}")
    ...
    def __chat_func_clean_up(gpt_response, prompt=""):  ####
        ...
        logger.warning(f"gpt_response: {gpt_response}")
...
def run_gpt_prompt_summarize_ideas(
    ...
    def create_prompt_input(persona, statements, question, test_input=None):
        prompt_input = [statements, persona.name, question]
        return prompt_input
...
def run_gpt_prompt_generate_whisper_inner_thought(
    ...
    def create_prompt_input(persona, whisper, test_input=None):
        prompt_input = [persona.name, whisper]
        return prompt_input
...
def run_gpt_generate_safety_score(persona, comment, test_input=None, verbose=False):
    ...
    output = ChatGPT_safe_generate_response_OLD(
        prompt, 3, fail_safe, __chat_func_validate, __chat_func_clean_up, verbose=False
    )
...
def run_gpt_generate_iterative_chat_utt(
    ...
    def __chat_func_validate(gpt_response, prompt=""):
        try:
            ...
            logger.debug(extract_first_json_dict(gpt_response))
            # print ("debug 3")
